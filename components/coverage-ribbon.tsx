        ...
        <div className="coverage-ribbon__grid">
          {items.map((item) => (
            <div key={`${item.label}-${item.value}`} className="coverage-ribbon__item">
              ...
              <p className="mt-1 text-xs font-black uppercase tracking-[0.18em] text-ink/54">{item.label}</p>
            </div>
          ))}
        </div>
        {footnote ? <div className="text-xs leading-6 text-ink/58">{footnote}</div> : null}
      </div>
    </section>
  );
}
