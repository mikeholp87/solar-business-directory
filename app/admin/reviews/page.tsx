  ...
  return (
    <section className="grid gap-4">
      <div className="surface-card p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-navy/65">Approve customer reviews before they appear on installer profiles.</p>
      </div>
        ...
        <form key={review.id ?? `${review.installerId}-${review.customerName}`} action={saveReviewAction} className="surface-card grid gap-4 p-5">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-navy/65">{review.rating}/5 · {review.installerId}</p>
            </div>
            <span className={review.approved ? "chip chip-success" : "chip chip-soft"}>{review.approved ? "Approved" : "Pending"}</span>
          </div>
          <p className="leading-7 text-navy/70">{review.reviewText}</p>
